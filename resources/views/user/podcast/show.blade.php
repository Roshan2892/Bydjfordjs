    ...
    <div class="col-md-offset-3 col-md-8 col-md-offset-2">
        @if(!empty($podcast))
            @foreach($podcast as $details)
                <img src="{!! \Illuminate\Support\Facades\Storage::url('uploads/images/'.$details->poster) !!}" width="250" height="150" alt="Poster">
                <h1>{{ title_case($details->title) }}</h1>
                ...
                <a href="{{ route('podcast.download',[ 'id'=> $details->id ]) }}" class="btn btn-default">Download</a>
            @endforeach
        @endif
    </div>  
@endsection