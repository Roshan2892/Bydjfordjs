@extends('user.layouts.app')
@section('title')
    PODCAST
@endsection
@section('content')
    <div class="media_container">
        <h1>Latest Podcast</h1><hr>
     <div class="data_slider">
    @if(count($podcasts) > 0)
        @foreach($podcasts as $podcast)
            <!-- start row -->
                <div class="row media_content">
                    <div class="col-lg-5 col-sm-5 col-xs-12">
                        <div class="image">
                            <a href="{{ route('podcast.show',[ 'title' => $podcast->seo_title ]) }}">
                                <img src="{!!\Illuminate\Support\Facades\Storage::url('uploads/images/'.$podcast->poster) !!}" alt="" height="300" width="350">
                            </a>
                        </div>
                    </div>
                    <div class="col-lg-7 col-sm-7 col-xs-12">
                        <div class="description">
                            <h1>
                                <a href="{{ route('podcast.show', [ 'title' => $podcast->seo_title ]) }}">
                                    {{ strlen($podcast->title) > 40 ? substr($podcast->title, 0, 40) . ' ..' : $podcast->title }}
                                </a>
                            </h1>
                            <h5 class="music_desc">
                                {!! strlen($podcast->description) > 150 ? substr($podcast->description, 0, 150) . ' ..' : $podcast->description  !!}
                            </h5>
                            <h5 class="music_artist"><b>Artist:</b> {{ $podcast->artist }}</h5>
                            <h5 class="music_tags">
                                <b>Tags:</b>
                                @foreach(unserialize($podcast->tags) as $tag)
                                    {{ $tag }}
                                @endforeach
                            </h5>
                            <h5 class="music_lang">{{ title_case($podcast->language) }}</h5>
                        </div>
                    </div>
                </div>
            @endforeach
            {{ $podcasts->links() }}
        @else
             <div class='coming_soon'>
               <img src="../image/assets/comingsoon.jpg" title='Coming Soon' alt="COMING SOON" width='100%' height="400">
            </div>
        @endif
      </div>
    </div>
@endsection
